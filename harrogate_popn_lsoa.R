  ...
  filter(age_group %in% c("Aged 0 to 15", "Aged 16 to 64", "Aged 65+")) %>% 
    # fct_reorder(lsoa_label, population, .desc = TRUE)
      left_join(y = age_order, by = "lsoa_label") %>% 
        select(-age_group.y) %>% 
          rename(total_popn = population.y, bracket_popn = population.x) %>% 
...
quartiles <- round(quantile(popn_data$population, probs = seq(1, 0, length = 5), names = TRUE))
